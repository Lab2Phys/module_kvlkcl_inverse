...
def add_voltage_table_page(pdf, table_data, unknown_value, unknown_name, decimal_places):
    ...
    t = ax.table(cellText=table_data, colLabels=["Nodes", "Voltage (V)"],
                loc='center', colWidths=[0.15, 0.15])
...
def add_current_table_page(pdf, table_data, unknown_value, unknown_name, decimal_places):
    ...
    t = ax.table(cellText=table_data, colLabels=["Branch", "Current (mA)", "Direction", "Resistance (Ω)"],
                loc='center', colWidths=[0.12, 0.12, 0.12, 0.12])
    t.auto_set_font_size(False)
    t.set_fontsize(10)
    t.scale(1, 1.2)
        ...
        with output_area:
            ...
            try:
                ...
                if sign_choice == 'both':
                    ...
                    print("\nCurrent Table:")
                    print(tabulate(current_table, headers=["Branch", "Current (mA)", "Direction", "Resistance (Ω)"],
                                  tablefmt="fancy_grid", numalign="center"))
