...
def calculate_potentials(unknown_value, voltage_source_config, n, loops, Z_matrix,
    ...
    for i, loop_nodes in enumerate(loops):
        ...
        # [FIX for Current Direction]
        # Correct the sign convention for the KVL equation (Z*I = V).
        # V must represent the net voltage drop from sources, which is the negative of voltage gain.
        V[i] = -loop_voltage
...
def make_combined_table(voltage_table, current_table, decimal_places=4):
    ...
    for node_pair, v in voltage_table:
        # To handle node pairs like (1, 2) vs (2, 1)
        node_pair_key_rev = f"({node_pair.split(',')[1].strip()}, {node_pair.split(',')[0].strip('() ')})"
        if node_pair in current_map:
            i, direction = current_map[node_pair]
        elif node_pair_key_rev in current_map:
            i, direction = current_map[node_pair_key_rev]
        else:
            i, direction = "–", "–"
...
def create_analyzer_ui(n, voltage_source_config, edges, loops, decimal_places=4, num_nodes=8):
    ...
    # Find unknown source based on the new tuple format ('unknown', sign)
