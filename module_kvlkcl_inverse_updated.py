...
def calculate_potentials(unknown_value, voltage_source_config, n, loops, Z_matrix,
    ...
    # --- FIXED: Store both directions of branch currents ---
    ...
    for i, loop_nodes in enumerate(loops):
        ...
        for u, v in current_edges:
            ...
            branch_currents[(v, u)] -= loop_current  # opposite direction
    ...
    while queue:
        ...
        for v in graph[u]:
            if v not in visited:
                i_uv = branch_currents.get((u, v), 0.0)
...
def make_current_table(branch_currents, edges, decimal_places=4):
    table = []
    for (u, v, r) in edges:
        i_uv = branch_currents.get((u, v), 0.0)  # direct direction u→v
