...
def add_combined_table_page(pdf, table_data, unknown_value, unknown_name, decimal_places):
    ...
    ax.axis('off')
    ax.set_title(f"Voltage and Current Table for {unknown_name} = {unknown_value:.{decimal_places}f} V",
...
def create_analyzer_ui(n, voltage_source_config, edges, loops, decimal_places=4, num_nodes=8):
    ...
    node_pair_dropdown = widgets.Dropdown(
        options=node_pairs, value=node_pairs[0], description='Node Pair:',
        layout=widgets.Layout(width='300px')
    )
    voltage_magnitude_box = widgets.FloatText(
        value=0.01, description='Voltage (V):',
        layout=widgets.Layout(width='200px')
    )
    voltage_sign_dropdown = widgets.Dropdown(
        options=[('V₁ > V₂ (Positive)', '+'),
                 ('V₂ > V₁ (Negative)', '-'),
                 ('Check Both Cases', 'both')],
        value='both', description='Direction:',
        layout=widgets.Layout(width='250px')
    )
    calc_button = widgets.Button(
        description="Calculate", button_style='success',
            ...
            try:
                ...
                if sign_choice == 'both':
                    ...
                    with PdfPages(pdf_filename) as pdf:
                        ...
                        print(tabulate(combined_pos,
                                       headers=["Nodes", "Voltage (V)", "Current (mA)", "Direction"],
