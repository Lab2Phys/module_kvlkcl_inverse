...
# --- Modified calculate_potentials to handle current directions correctly ---
    ...
    for i, loop_nodes in enumerate(loops):
        ...
        for u, v in current_edges:
            if not isinstance(vs.get((u, v)), tuple):
                loop_voltage -= vs.get((u, v), 0)  # Negative if (u,v) matches source direction
            if not isinstance(vs.get((v, u)), tuple):
                loop_voltage += vs.get((v, u), 0)  # Positive if (v,u) matches source direction
        V[i] = loop_voltage
    ...
    while queue:
        ...
        for v in graph[u]:
            if v not in visited:
                ...
                v_gain_e = -vs.get((u, v), 0.0) if not isinstance(vs.get((u, v)), tuple) else 0.0
                v_gain_e += vs.get((v, u), 0.0) if not isinstance(vs.get((v, u)), tuple) else 0.0
                potentials[v] = potentials[u] - v_drop_r + v_gain_e
                visited.add(v)
                queue.append(v)
    return potentials, branch_currents

# --- Modified find_unknown_voltage ---
...
# --- Modified make_current_table to fix direction ---
